...
import { ReqCustomer } from './../customer/models/req.customer.model';
...
export class CustomerController {
  ...
  async get(@Req() req: Request, @Param('id') id: string) {
    const result = await this.customerService.get(req, id);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
  ...
  async create(
    @Req() req: Request,
    @Body() body: ReqCustomer,
  ): Promise<Customer | Problem> {
    const result = this.customerService.create(req, body);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
  ...
  async update(
    ...
    const result = this.customerService.update(req, id, body);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }

  @Delete('delete/:id')
  async delete(@Req() req: Request, @Param('id') id: string): Promise<any> {
    const result = this.customerService.delete(req, id);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
}
