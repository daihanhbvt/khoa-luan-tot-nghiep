...
import { ReqFloors } from './../floors/models/req.floors.model';
...
export class FloorsController {
  ...
  async all(@Req() req: Request, @Query() query: Pagination) {
    const result = await this.floorsService.findAll(req, new Pagination(query));
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }

  @Get('get/:id')
  async get(@Req() req: Request, @Param('id') id: string) {
    const result = await this.floorsService.get(req, id);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
  ...
  async create(
    ...
    const result = this.floorsService.create(req, body);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
  ...
  async update(
    ...
    const result = this.floorsService.update(req, id, body);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }

  @Delete('delete/:id')
  async delete(@Req() req: Request, @Param('id') id: string): Promise<any> {
    const result = this.floorsService.delete(req, id);
    return result instanceof Problem ? Problem.HttpException(result) : result;
  }
}
